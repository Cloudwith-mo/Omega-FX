...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze live MT5 trades vs simulated eval runs.")
    parser.add_argument("--live_trades_csv", type=Path, required=True, help="MT5 export CSV (detailed report).")
    parser.add_argument(
        "--sim_runs_csv",
        type=Path,
...
def load_live_trades(path: Path) -> pd.DataFrame:
    ...
    timestamp_col = next((col for col in df.columns if col.lower() in {"time", "timestamp", "date"}), None)
    if timestamp_col is None:
        raise ValueError("Live trades CSV must include a timestamp column (Time/Timestamp/Date).")
...
def _coalesce_columns(df: pd.DataFrame, candidates: list[str], default: float = 0.0) -> pd.Series:
...
def compute_live_metrics(df: pd.DataFrame, account_size: float, reference_risk: float) -> dict:
    cum_pnl = df["pnl"].cumsum()
    equity = account_size + cum_pnl
    total_return_pct = 0.0 if equity.empty else ((equity.iloc[-1] - account_size) / account_size) * 100.0
    ...
    avg_r = 0.0 if num_trades == 0 or reference_risk <= 0 else float((df["pnl"] / reference_risk).mean())
...
def load_sim_runs(path: Path, account_size: float) -> dict:
    ...
    max_dd = _coalesce_sim_fraction(df, ["max_trailing_dd_fraction", "max_drawdown_fraction"]) * 100.0
    max_daily = _coalesce_sim_fraction(df, ["max_observed_daily_loss_fraction", "max_daily_loss_fraction"]) * 100.0
    return {
...
def build_comparison(live: dict, sim: dict) -> dict:
    return {
        "return_percentile": percentile_rank(sim["return_pct"], live["total_return_pct"]),
        "max_drawdown_percentile": percentile_rank(sim["max_dd_pct"], live["max_drawdown_pct"]),
        "max_daily_loss_percentile": percentile_rank(sim["max_daily_loss_pct"], live["max_daily_loss_pct"]),
...
def main() -> int:
    ...
    live_metrics = compute_live_metrics(live_df, args.account_size, args.reference_risk_per_trade)
