...
from scripts.build_notification_snapshot import build_notification_snapshot  # noqa: E402
...
def main() -> int:
    ...
    steps = [
        ("pytest execution backend", [sys.executable, "-m", "pytest", "tests/test_execution_backend.py"]),
        ("MT5 smoketest", [sys.executable, "scripts/run_exec_mt5_smoketest.py", "--account_profile", "METAQUOTES_DEMO", "--dry_run"]),
        (
    ...
    if not failures:
        try:
            report_6h = generate_exec_report(Path("results/mt5_demo_exec_log.csv"), 6, "demo")
            report_24h = generate_exec_report(Path("results/mt5_demo_exec_log.csv"), 24, "demo")
    ...
    if not failures:
        try:
            _, snapshot_path = build_notification_snapshot(tag="demo", results_dir=RESULTS_DIR, hours_fast="6", hours_slow="24")
        except Exception as exc:
